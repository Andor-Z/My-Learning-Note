...
def index():
    ...
    # ?????????§Ö????? ??? all() ???? Flask-SQLAlchemy ???? paginate() ????
    # ????? paginate() ???????????????????¦·????????????
    # ??????? per_page ?????????????????????? ???????????????????? 20 ???????
    # ???????? error_out????????? True ???????????????????????????????¦¶???????? 404 ????????????? False???????????¦¶???????????§Ò???
...
def show_all():
    ...
    return resp 
    # set_cookie() ???????????????????? cookie ???????????? max_age ???????? cookie ??????????¦Ë???????????????? max_age???????????? cookie ??????????????§µ????????? 30 ??
...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        # ????? author?????????????? current_user???????????????????????????????????? User ???????????? current_user._get_current_object() ?????
...
#         return redirect(url_for('.index'))  # ???????
#         # Flask ????????§Ö??????????????????????????????????????????????????????????????????? ????????????????????????????????????? Blueprint ??????????????????????????????? index() ??????????? main.index???? URL ??? url_for('main.index') ?????
